...
module.exports = {
    ...
    async run(client, message, args) {
        ...
        try {
            ...
            return sendV2Message(client, message.channel.id, await t('moderation.addrole_success', message.guild.id, { role: role.name, user: targetMember.user.tag }), []);
        } catch (err) {
            console.error(err);
            return sendV2Message(client, message.channel.id, await t('moderation.addrole_error', message.guild.id), []);
        }
    }
};
