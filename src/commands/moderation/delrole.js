...
module.exports = {
    name: 'delrole',
    description: 'delrole.description',
    category: 'Moderation',
    usage: 'delrole.usage',
    async run(client, message, args) {
        ...
        const replyMsg = await message.channel.send({ embeds: [createEmbed(await t('common.loading_title', message.guild.id), `${THEME.icons.loading} ${await t('moderation.delrole_process', message.guild.id)}`, 'loading')] });

        try {
            await targetMember.roles.remove(role);
            await replyMsg.edit({ embeds: [createEmbed(await t('common.success_title', message.guild.id), await t('moderation.delrole_success', message.guild.id, { role: role.name, user: targetMember.user.tag }), 'success')] });
        } catch (err) {
            console.error(err);
            await replyMsg.edit({ embeds: [createEmbed(await t('common.error_title', message.guild.id), await t('moderation.delrole_error', message.guild.id), 'error')] });
        }
    }
};
